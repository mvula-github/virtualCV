export const SKILLS = [
    {
        title: "Frontend",
        icon: "./assets/monitor.png",
        skills: [
            {skill: "HTML", percentage: "72%" },
            {skill: "css", percentage: "79%" },
            {skill: "Javascript", percentage: "77%" },
            {skill: "React.js", percentage: "80% " },
            {skill: "Figma", percentage: "60%" },
        ],
    },
    {
        title: "Backend",
        icon: "./assets/backend.png",
        skills: [
            {skill: "C/C++", percentage: "67%" },
            {skill: "C#", percentage: "89%" },
            {skill: "Java", percentage: "86%" },
            {skill: "Python", percentage: "67%" },
            {skill: "MySQL", percentage: "70%" },
            {skill: "Express.js", percentage: "5%" },
        ],
    },
    {
        title: "Tools",
        icon: "./assets/technical-support.png",
        skills: [
            {skill: "VS Code", percentage: "70%" },
            {skill: "Visual Studio", percentage: "72%" },
            {skill: "Git and Github", percentage: "60%" },
            {skill: "Cisco (Packet Tracer)", percentage: "53%" },
            {skill: "Microsoft Office", percentage: "83%" },
            {skill: "Google Chrome", percentage: "90%" },
        ],
    },

    {
        title: "Soft Skills",
        icon: "./assets/doctor.png",
        skills: [
            {skill: "Communication", percentage: "78%" },
            {skill: "Attentive", percentage: "89%" },
            {skill: "Problem-solving", percentage: "80%" },
            {skill: "Teamwork", percentage: "82%" },
            {skill: "Punctual", percentage: "98%" },
            {skill: "Analytical", percentage: "80%" },
        ],
    }
]

export const WORK_XP = [
    {
        title:"Student Assistant || North-West University ",
        date:"July 2023 - October 2023",
        duties:[
            "Attend lectures and provide support to students.",
            "Provide academic support to students.",
            "Grade assignments and provide feedback.",
        ],
    },
    {
        title:"Student Assistant || North-West Univerity",
        date:"July 2024 - Present",
        duties:[
            "Grade formal tests and assignments and provide feedback.",
            "Provide academic support to students.",
        ],
    },
]